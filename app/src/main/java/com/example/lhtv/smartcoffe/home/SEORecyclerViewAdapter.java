...
public class SEORecyclerViewAdapter extends RecyclerView.Adapter<SEOViewHolder> {
    ...
    private final LinkedList<TableDrink> itemList;
    private final OnItemClickListener listener;
    private Context context;
    public SEORecyclerViewAdapter(Context context, LinkedList<TableDrink> list,OnItemClickListener listener){
...
//        }
        ...
        for (int i = 0; i< Instance.billInfoList.size();i++){
            if(item.id == Instance.billInfoList.get(i).bill_id && Instance.billInfoList.get(i).count != 0 ){
                count ++;
            }
        }
        if(count != 0){
